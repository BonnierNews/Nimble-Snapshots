...
public func equal<T>(_ expectedValue: Set<T>) -> Nimble.Predicate<Set<T>> {
...
/// A Nimble matcher that succeeds when the actual set is equal to the expected set.
public func equal<T: Comparable>(_ expectedValue: Set<T>) -> Nimble.Predicate<Set<T>> {
    equal(expectedValue as Set<T>?)
}
...
private func equal<T>(_ expectedValue: Set<T>?, stringify: @escaping (Set<T>?) -> String) -> Nimble.Predicate<Set<T>> {
...
public func equal<K: Hashable, V: Equatable>(_ expectedValue: [K: V?]) -> Nimble.Predicate<[K: V]> {
    Predicate.define("equal <\(stringify(expectedValue))>") { actualExpression, msg in
        guard let actualValue = try actualExpression.evaluate() else {
            return PredicateResult(
                status: .fail,
