...
public func beLessThanOrEqualTo<T: Comparable>(_ expectedValue: T?) -> Nimble.Predicate<T> {
...
/// A Nimble matcher that succeeds when the actual value is less than
/// or equal to the expected value.
public func beLessThanOrEqualTo<T: NMBComparable>(_ expectedValue: T?) -> Nimble.Predicate<T> {
    return Predicate.simple("be less than or equal to <\(stringify(expectedValue))>") { actualExpression in
